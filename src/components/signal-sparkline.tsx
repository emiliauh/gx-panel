...
export function SignalSparkline({
  ...
  // Use a fixed viewBox width for calculations, SVG will scale
  ...
  const pathD = useMemo(() => {
    ...
    const points = data.map((value, index) => {
      ...
      const y = viewBoxHeight - ((value - min) / range) * (viewBoxHeight - 4) - 2
      return `${x},${y}`
    })
  ...
  const areaPathD = useMemo(() => {
    ...
    return `M 0,${viewBoxHeight} L ${points.join(" L ")} L ${viewBoxWidth},${viewBoxHeight} Z`
  }, [data, viewBoxHeight])

  if (data.length < 2) {
    return (
      <div
        className={cn("flex items-center justify-center text-xs text-muted-foreground h-10", className)}
      >
        Collecting...
      </div>
    )
  ...
  return (
    <svg
      className={cn("overflow-visible w-full h-auto", className)}
