...
export default function DevicesPage() {
  ...
  // Memoize expensive client processing to avoid re-computing on every render
  const allClients: ClientWithType[] = useMemo(() => {
  ...
  const getTypeBadge = (type: string) => {
    switch (type) {
      ...
      case "5.0ghz":
        return <Badge variant="blue">5 GHz</Badge>
      default:
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Connected Devices</h1>
            ...
            <>
              ...
              <div className="space-y-3 md:hidden">
                {uniqueClients.map((client) => (
                  <div key={client.mac} className="p-4 rounded-xl bg-muted/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <code className="text-xs text-muted-foreground">{client.ipv4}</code>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      ...
                      {client.signal !== undefined && (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-muted-foreground">{client.signal} dBm</span>
                        </div>
                      )}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      MAC: <code className="text-xs">{client.mac}</code>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="rounded-xl overflow-hidden border border-border/50 hidden md:block">
