...
export default function SystemPage() {
  ...
  const handleReboot = async () => {
    setRebooting(true)
    setShowConfirm(false) // Hide confirmation, show rebooting state
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Reboot failed:", errorData)
        setRebooting(false) // Only reset if failed
      }
      // On success, keep rebooting=true to show the message
      // The gateway will be unavailable, so we don't reset the state
    } catch (error) {
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between p-3 rounded-xl bg-muted/30">
                  ...
                  <Badge variant="blue">v{device.softwareVersion}</Badge>
                </div>
                {apiVersion && (
                  <div className="flex justify-between p-3 rounded-xl bg-muted/30">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-4">
              {!showConfirm ? (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 bg-destructive/10 rounded-xl border border-destructive/20 w-full">
